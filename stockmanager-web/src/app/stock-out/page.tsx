...
export default function StockOutPage() {
  const [stockOutData, setStockOutData] = useState<出库数据[]>([])
  const [filteredData, setFilteredData] = useState<出库数据[]>([])
    ...
    <div className="p-6">
      ...
      <div className="bg-white dark:bg-zinc-800 p-4 rounded shadow overflow-auto">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-lg font-semibold text-gray-800 dark:text-gray-200">出库数据表</h2>
          <span className="text-sm text-gray-500 dark:text-gray-400">
            显示 {filteredData.length} / {stockOutData.length} 条记录
          </span>
        </div>
